...
- **Auth wiring TODO (manual, dashboard-only):** one thing needs to be turned on in the Supabase dashboard for the project (no MCP tool covers it):
  1. Authentication → Hooks (Beta) → Custom Access Token: select `public.custom_access_token_hook`. Until this is enabled, JWTs won't carry `contact_id`/`tenant_id`, and `AppModeResolver` will always fall back to Prospect mode. (Email auth itself needs no separate provider setup — Supabase sends confirmation/OTP emails via its own built-in mailer by default.)
