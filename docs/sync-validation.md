...
- **Review link needs a real Google Place ID before launch:** `NYSLinks.googlePlaceID` is nil, so the button currently falls back to a Maps search for "New York Sash, 349 Oriskany Blvd, Whitesboro NY". Fill it in from Google's Place ID Finder to open the write-a-review dialog directly. Left nil deliberately rather than guessed — a wrong ID sends customers to another business's review form.
