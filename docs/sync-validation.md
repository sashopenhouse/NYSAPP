...
- **Auth wiring TODO (manual, dashboard-only):** two things need to be turned on in the Supabase dashboard for the project (no MCP tool covers either):
  1. Authentication → Providers → Phone: enable phone auth and configure a real SMS provider (Twilio/MessageBird/Vonage) with account credentials. Until this is on, `signInWithOTP(phone:)` in the iOS app cannot actually send a text.
  2. Authentication → Hooks (Beta) → Custom Access Token: select `public.custom_access_token_hook` (added in migration `0002_auth_hook.sql`, search_path pinned in `0003_auth_hook_search_path.sql`). Until this is enabled, JWTs won't carry `contact_id`/`tenant_id`, and `AppModeResolver` will always fall back to Prospect mode.
